CREATE FOREIGN TABLE pypi_dim_next.downloads_data_set (
...
CREATE OR REPLACE FUNCTION pypi_tmp.create_downloads_data_set(day_chunk SMALLINT)
  RETURNS SETOF pypi_dim_next.DOWNLOADS_DATA_SET AS $$
SELECT
...
CREATE OR REPLACE FUNCTION pypi_tmp.insert_downloads_data_set(day_chunk SMALLINT)
  RETURNS VOID AS $$
BEGIN
  EXECUTE 'CREATE TABLE pypi_tmp.downloads_data_set_' || day_chunk
          || ' AS SELECT * FROM pypi_tmp.create_downloads_data_set(' || day_chunk || '::SMALLINT );';

  EXECUTE 'INSERT INTO pypi_dim_next.downloads_data_set SELECT * FROM pypi_tmp.downloads_data_set_'
          || day_chunk || ';';

  EXECUTE 'DROP TABLE pypi_tmp.downloads_data_set_' || day_chunk;
